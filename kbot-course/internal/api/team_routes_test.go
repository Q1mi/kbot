...
func TestWriteTeamRunResponseAwaitingApproval(t *testing.T) {
	...
	writeTeamRunResponse(recorder, "", []runtimeteam.Step{{Role: "billing", AgentID: "agent-1"}},
...
func TestWriteTeamRunResponseFailure(t *testing.T) {
	...
	if recorder.Code != http.StatusBadGateway || !strings.Contains(recorder.Body.String(), "team failed") {
