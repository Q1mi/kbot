...
export function ConversationsPage() {
  ...
  async function restoreConversation(id: string) {
    try {
      ...
      let restoredSurfaces: A2UISurfaces = {}
      const restoredSurfaceItems = (detail.a2ui_surfaces ?? []).flatMap((history) => {
        restoredSurfaces = history.messages.reduce(applyA2UIMessage, restoredSurfaces)
        const surfaceId = history.messages.find((item) => item.createSurface)?.createSurface?.surfaceId
        return surfaceId ? [{
          timestamp: history.created_at,
          item: { kind: 'surface' as const, id: `surface-${surfaceId}`, surfaceId },
        }] : []
      })
      ...
      setItems([...restoredTurnItems, ...restoredSurfaceItems]
