...
curl -fsS -N -X POST "$base_url/stream/agents/$agent_id/chat" "${auth[@]}" \
  -d '{"message":"审核高风险案件 CLM-2026-0002 并冻结赔款"}' >"$stream_file"
...
if ! sed -n 's/^data: //p' "$stream_file" | jq -e 'select(.type=="a2ui") | select((.data|tostring)|contains("欺诈风险冻结审批"))' >/dev/null; then
  echo "approval stream did not include insurance A2UI metadata" >&2
  exit 1
fi
...
for _ in $(seq 1 15); do
  ...
  if printf '%s' "$conversation" | jq -e '.messages[]? | select(.role=="assistant" and (.content|contains("赔款已经冻结")))' >/dev/null; then
    completed=true
    break
  fi
  sleep 1
...
dataset_id=$(curl -fsS "$base_url/api/v1/eval/datasets" "${auth[@]}" | jq -r '.[] | select(.name=="insurance-offline-golden") | .id' | head -1)
